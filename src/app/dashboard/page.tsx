...
export default function DashboardPage() {
  ...
  const [expectedReturn, setExpectedReturn] = useState("14.2% ARR");
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setCapital(parsed.capital || "10,00,000");
      setRiskProfile(parsed.riskAppetite || "Moderate");

      if (parsed.riskAppetite === "Conservative") {
        setExpectedReturn("9.8% ARR");
        setSharpeRatio("1.85");
        setVolatility("6.2%");
        setConfidenceScore("89%");
        setFrontierPoints({
          current: [{ x: 8.5, y: 6.8, name: "Current Portfolio" }],
          optimized: [{ x: 6.2, y: 9.8, name: "Optimized Portfolio" }],
        });
        setAllocations([
          { name: "TCS", value: 15, color: "#0E8A5A", expectedReturn: "14%", confidence: "84%", momentum: "Positive", eps: "Growing", revenue: "Increasing", reason: "Stable yield equities." },
          { name: "Infosys", value: 10, color: "#0F172A", expectedReturn: "12.5%", confidence: "82%", momentum: "Positive", eps: "Growing", revenue: "Stable", reason: "Stable tech titan adding low-covariance growth to the portfolio frontier." },
          { name: "Reliance", value: 15, color: "#3B82F6", expectedReturn: "13.8%", confidence: "85%", momentum: "Positive", eps: "Stable", revenue: "Increasing", reason: "Defensive heavy weight providing core systemic stability." },
          { name: "Gold ETF", value: 30, color: "#F59E0B", expectedReturn: "8.5%", confidence: "90%", momentum: "Neutral", eps: "N/A", revenue: "N/A", reason: "Acts as a negative-correlation hedge protecting against equity drawdown events." },
          { name: "Nifty ETF", value: 30, color: "#8B5CF6", expectedReturn: "11.2%", confidence: "88%", momentum: "Positive", eps: "N/A", revenue: "N/A", reason: "Provides broad beta market indexing and mitigates single-stock volatility." },
        ]);
        setChatLog([
          { sender: "ai", text: "Welcome to your Conservative Quant Advisor agent. Your capital is allocated mostly to safe index ETFs and Gold to preserve wealth while targeting 9.8% ARR." }
        ]);
      } else if (parsed.riskAppetite === "Aggressive") {
        setExpectedReturn("18.5% ARR");
        setSharpeRatio("1.22");
        setVolatility("14.8%");
        setConfidenceScore("76%");
        setFrontierPoints({
          current: [{ x: 12.5, y: 12.8, name: "Current Portfolio" }],
          optimized: [{ x: 14.8, y: 18.5, name: "Optimized Portfolio" }],
        });
        setAllocations([
          { name: "TCS", value: 40, color: "#0E8A5A", expectedReturn: "14%", confidence: "84%", momentum: "Positive", eps: "Growing", revenue: "Increasing", reason: "High-weight market leader." },
          { name: "Infosys", value: 25, color: "#0F172A", expectedReturn: "12.5%", confidence: "82%", momentum: "Positive", eps: "Growing", revenue: "Stable", reason: "Aggressively targets high-beta growth weights." },
          { name: "Reliance", value: 20, color: "#3B82F6", expectedReturn: "13.8%", confidence: "85%", momentum: "Positive", eps: "Stable", revenue: "Increasing", reason: "Core market giant." },
          { name: "Gold ETF", value: 5, color: "#F59E0B", expectedReturn: "8.5%", confidence: "90%", momentum: "Neutral", eps: "N/A", revenue: "N/A", reason: "Hedge reduced to maximize growth opportunities." },
          { name: "Nifty ETF", value: 10, color: "#8B5CF6", expectedReturn: "11.2%", confidence: "88%", momentum: "Positive", eps: "N/A", revenue: "N/A", reason: "Provides index diversification support." },
        ]);
        setChatLog([
          { sender: "ai", text: "Welcome to your Aggressive Quant Advisor agent. We have tilted allocations heavily towards high-beta equities (TCS + Infosys at 65%) to target high growth yield." }
        ]);
      } else {
        // Moderate (default)
        setExpectedReturn("14.2% ARR");
        setSharpeRatio("1.54");
        setVolatility("9.4%");
  ...
  const handleRagSearch = () => {
    ...
    setTimeout(() => {
      let answer = "";
      if (ragQuestion.toLowerCase().includes("tcs")) {
        answer = "Source: TCS Q4 Annual Report (Page 42).\n\"Revenue increased 15% with healthy digital services margins. Low debt-to-equity ratio supports continued capital expenditure. SEBI risk compliance indices validated.\"";
      } else if (ragQuestion.toLowerCase().includes("sebi") || ragQuestion.toLowerCase().includes("rule")) {
        answer = "Source: SEBI Mutual Fund Guidelines (Section 7).\n\"No single asset in a balanced quant mandate shall exceed 35% concentration threshold, ensuring systematic diversification.\"";
      } else {
        answer = "Source: Portfolio Theory Textbook (Page 112).\n\"Diversification reduces unsystematic stock risk. Gold ETF serves as a hedging hedge against equity drawdowns.\"";
      }
      setRagAnswer(answer);
  ...
  const handleChatSend = (text: string) => {
    ...
    setTimeout(() => {
      ...
      if (q.includes("risk")) {
        reply = `Your risk profile is currently set to '${riskProfile}'. Adjusting to a higher mandate would shift weights into high-beta equities (TCS/Infosys), increasing returns but stretching volatility limits.`;
      } else if (q.includes("infosys") || q.includes("infy")) {
        reply = `Infosys is allocated at ${allocations.find(a => a.name.includes("Infosys"))?.value || 20}%. Our MPT optimizer solved this target based on standard deviations and risk constraints.`;
      } else if (q.includes("inflation")) {
        reply = "If inflation indices rise, Gold ETF (GOLDBEES) acts as a hedge. The live risk monitors will auto-shift weights from equities into Gold to protect purchasing power.";
      } else if (q.includes("rebalance")) {
        reply = "The current weight drift is minor. Rebalancing is recommended if target deviations exceed 3.0% in single stock allocations.";
      } else {
        reply = `Your request was processed by the decision engine. Under your current active ${riskProfile} mandate, we maintain optimal Sharpe allocations.`;
      }
